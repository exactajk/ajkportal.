...
const DEFAULT_RECORDS = [
    ...
    },
    {
        ...
        age: 32,
        gender: "Male",
        cnic: "82701-7890123-7",
        district: "Poonch",
        village: "Hajira",
        address: "Hajira Hospital Ward, Poonch",
        location: "Hajira, Poonch",
        status: "injured",
        date: "2026-07-14",
        time: "11:30 AM",
        lastSeen: "Poonch Highway",
        details: "Under medical care at District Hospital. Stable condition.",
        image: "assets/images/irfan.png",
        reporterName: "Mahmood Ahmed",
        reporterPhone: "0346-4443322",
    ...
    },
    {
        ...
        age: 26,
        gender: "Female",
        cnic: "82801-8901234-8",
        district: "Muzaffarabad",
        village: "CMH Ward",
        address: "CMH Muzaffarabad",
        location: "CMH Muzaffarabad",
        status: "injured",
        date: "2026-07-16",
        time: "03:45 PM",
        lastSeen: "Muzaffarabad City",
        details: "Receiving treatment at CMH. Seeking relatives.",
        image: "assets/images/ali.png",
        reporterName: "Muhammad Farooq",
        reporterPhone: "0301-9988776",
        reporterEmail: "amna@example.com",
        reporterRelation: "Brother",
        userSubmitted: false,
        approved: true,
        rejected: false
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize Forgot Password OTP Workflow
    initForgotPasswordWorkflow();
...
function setWizardReportType(type) {
    ...
    if (radioInput) {
        ...
        // Update date & location labels based on report type
        const dateLabel = document.getElementById('lbl-date');
        const locationLabel = document.getElementById('lbl-location') || document.querySelector('label[for="person-location"]');
        ...
        if (dateLabel) {
            if (type === 'missing') {
                ...
                dateLabel.innerHTML = 'Date of Incident (Date of Injury) <span class="required">*</span>';
            } else {
        ...
        if (locationLabel) {
            if (type === 'injured') {
                locationLabel.innerHTML = 'Place of Injury / Hospital Name (کہاں زخمی ہوا/ہسپتال) <span class="required">*</span>';
                if (locationInput) locationInput.placeholder = 'e.g. Hajira Bazaar / CMH Hospital Ward';
            } else if (type === 'shaheed') {
                locationLabel.innerHTML = 'Place of Shahadat <span class="required">*</span>';
                if (locationInput) locationInput.placeholder = 'Enter sector or place of Shahadat';
            } else if (type === 'found') {
                locationLabel.innerHTML = 'Specific Location (Where Found) <span class="required">*</span>';
                if (locationInput) locationInput.placeholder = 'Enter town or village where found';
            } else {
                locationLabel.innerHTML = 'Specific Location (Last Seen Area) <span class="required">*</span>';
                if (locationInput) locationInput.placeholder = 'Enter town or village (last seen)';
            }
        }
    }
}
...
function renderHomeViews() {
    ...
    // 4. Recently Injured Persons (horizontal scroll slider cards)
    const injuredContainer = document.getElementById('recently-injured-container');
    if (injuredContainer) {
        const injuredRecords = appState.records.filter(r => r.status === 'injured' && r.approved === true).slice(0, 12);
        injuredContainer.innerHTML = '';
        
        if (injuredRecords.length === 0) {
            injuredContainer.innerHTML = '<div class="no-records">No injured reports registered.</div>';
        } else {
            injuredRecords.forEach(record => {
                const card = document.createElement('div');
                card.className = 'person-scroll-card injured-scroll-card';
                card.innerHTML = `
                    <div class="card-img-container">
                        <img src="${record.image || 'assets/images/logo.png'}" alt="${record.name}">
                    </div>
                    ...
                    <p>Age: ${record.age || 'N/A'} Years</p>
                    <p>District: ${record.district}</p>
                    <div class="date-badge" style="color: #b45309;">
                        <span>Injured Date:</span>
                        <strong class="date-val">${formatDate(record.date)}</strong>
                    </div>
                    <button class="card-details-btn" onclick="openDetails('${record.id}')">View Details</button>
                `;
                injuredContainer.appendChild(card);
            });
        }
    }
}
...
function performSearch() {
    ...
    let filtered = appState.records.filter(r => r.approved === true).filter(r => {
        ...
        // Search query filter (Report ID, Name, CNIC, District, Location)
        if (query) {
            const cleanQuery = query.replace('#', '');
            const idMatch = r.id ? r.id.toLowerCase().includes(cleanQuery) : false;
            const nameMatch = r.name ? r.name.toLowerCase().includes(cleanQuery) : false;
            const districtMatch = r.district ? r.district.toLowerCase().includes(cleanQuery) : false;
            const locationMatch = r.location ? (r.location || r.address || '').toLowerCase().includes(cleanQuery) : false;
            const cnicMatch = r.cnic ? r.cnic.includes(cleanQuery) : false;
            return idMatch || nameMatch || districtMatch || locationMatch || cnicMatch;
        }
...
async function openDetails(id) {
    ...
    if (record.status === 'found') {
        ...
        statusClass = 'pill-injured';
        statusLabel = 'Injured Person';
    } else {
        statusActionBtn = `
    ...
    const detailHTML = `
        ...
        <div class="detail-summary-list">
            <div class="detail-summary-item" style="background: #f8fafc; border: 1px dashed #cbd5e1;">
                <span class="label" style="font-weight: 700; color: #014d3b;">Report Case ID</span>
                <span class="value" style="font-weight: 800; font-family: monospace; letter-spacing: 1px; color: #014d3b;">#${(record.id || '').toUpperCase()}</span>
            </div>
            ...
            <div class="detail-summary-item">
                <span class="label">${record.status === 'injured' ? 'Place of Injury / Hospital' : (record.status === 'shaheed' ? 'Place of Shahadat' : (record.status === 'found' ? 'Where Found' : 'Last Seen Area'))}</span>
...
function initForgotPasswordWorkflow() {
    ...
    const closeFpBtn = document.getElementById('close-forgot-modal-btn');

    const btnSendOtp = document.getElementById('btn-fp-send-otp');
    const btnVerifyOtp = document.getElementById('btn-fp-verify-otp');
    const btnResetPass = document.getElementById('btn-fp-reset-pass');
    ...
    if (forgotLink) {
        forgotLink.addEventListener('click', (e) => {
            ...
            const fpNewPassEl = document.getElementById('fp-new-pass');
            const fpConfirmPassEl = document.getElementById('fp-confirm-pass');

            if (fpEmailEl) fpEmailEl.value = '';
            if (fpOtpEl) fpOtpEl.value = '';
            if (fpNewPassEl) fpNewPassEl.value = '';
            if (fpConfirmPassEl) fpConfirmPassEl.value = '';
    ...
    if (fpOverlay) fpOverlay.addEventListener('click', closeFpModalFn);
    if (closeFpBtn) closeFpBtn.addEventListener('click', closeFpModalFn);

    // Step 1: Send Password Reset Email to registered inbox
    if (btnSendOtp) {
        btnSendOtp.addEventListener('click', async () => {
            ...
            resetTargetEmail = emailInput;
            btnSendOtp.disabled = true;
            btnSendOtp.innerHTML = '<span class="sf-spinner"></span> <span>Sending Reset Link...</span>';

            // Trigger real Firebase Auth password reset email to user inbox
            ...
            btnSendOtp.innerHTML = '<i data-lucide="send"></i> <span>Send Email Verification Link</span>';

            showToast(`Password reset verification email sent to ${emailInput}! Please check your inbox.`, "success");
    ...
    const closeDoneBtn = document.getElementById('btn-fp-close-done');
    if (closeDoneBtn) {
        closeDoneBtn.addEventListener('click', closeFpModalFn);
    }
