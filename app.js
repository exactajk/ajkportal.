...
function initForgotPasswordWorkflow() {
    ...
    if (forgotLink) {
        forgotLink.addEventListener('click', (e) => {
            ...
            if (fpModal) fpModal.classList.add('open');
            safeCreateIcons();
        });
    }

    const closeFpModalFn = () => {
        if (fpModal) fpModal.classList.remove('open');
    };
    ...
    // Step 2: Verify & Reset Confirmation
    if (btnVerifyOtp) {
        btnVerifyOtp.addEventListener('click', () => {
            showToast("Verification confirmed. Set your new password below.", "success");
    ...
    if (btnResetPass) {
        btnResetPass.addEventListener('click', async () => {
            ...
            // Update local user accounts in localStorage
            let users = [];
            try {
                users = JSON.parse(localStorage.getItem('ajk_portal_users') || '[]');
            } catch (e) {}

            const userIndex = users.findIndex(u => u.email && u.email.toLowerCase() === resetTargetEmail.toLowerCase());
            if (userIndex !== -1) {
                users[userIndex].password = newPass;
                localStorage.setItem('ajk_portal_users', JSON.stringify(users));
            } else {
                users.push({ email: resetTargetEmail, password: newPass, name: resetTargetEmail.split('@')[0] });
                localStorage.setItem('ajk_portal_users', JSON.stringify(users));
            }

            // Firebase Password Reset Email if available
            if (typeof firebase !== 'undefined' && firebase.auth) {
                try {
                    await firebase.auth().sendPasswordResetEmail(resetTargetEmail);
                } catch (err) {
